...
# google-auth for Vertex AI / Gemini embeddings (OAuth2 service-account token)
RUN pip install --no-cache-dir google-auth==2.37.0 requests==2.32.3

# Optional: Gemini embeddings support (API-key based)
...
# Embedding provider: "openai", "gemini", or "vertex"
ENV LOCAL_MPS_EMBEDDING_PROVIDER=vertex
ENV LOCAL_MPS_EMBEDDING_MODEL=text-embedding-004
# Vertex AI settings (required when LOCAL_MPS_EMBEDDING_PROVIDER=vertex)
# ENV VERTEX_PROJECT_ID=your-gcp-project
ENV VERTEX_LOCATION=us-central1
ENV LOCAL_MPS_MODEL=gemini-3.5-flash
# Mount service-account JSON via volume and set GOOGLE_APPLICATION_CREDENTIALS
