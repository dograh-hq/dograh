...
def upgrade() -> None:
    ...
    sa.Enum('http_api', 'native', 'integration', name='tool_category').create(op.get_bind())
    ...
    sa.Column('category', postgresql.ENUM('http_api', 'native', 'integration', name='tool_category', create_type=False), nullable=False),
    sa.Column('icon', sa.String(length=50), nullable=True),
    sa.Column('icon_color', sa.String(length=7), nullable=True),
    sa.Column('status', postgresql.ENUM('active', 'archived', 'draft', name='tool_status', create_type=False), server_default=sa.text("'active'::tool_status"), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_table('tools')
    sa.Enum('http_api', 'native', 'integration', name='tool_category').drop(op.get_bind())
