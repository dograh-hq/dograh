...
class PapiVoipProvider(TelephonyProvider):
    ...
    PROVIDER_NAME = getattr(getattr(WorkflowRunMode, "PAPI_VOIP", None), "value", "papi_voip")
    ...
    async def verify_webhook_signature(
        ...
        """
        Verify incoming webhook signature from Papi callback.
        Supports HMAC-SHA256 signature verification or Bearer/API-key token match.
        """
        secret = (self.webhook_secret or self.api_key or "").strip()
        if not secret:
            return False
        ...
        # Fallback: if params contain matching instance_id
        if self.instance_id and params.get("instance_id") == self.instance_id:
            return True
    ...
    async def verify_inbound_signature(
        ...
        """
        Verify inbound webhook request signature using HMAC-SHA256, API key header, or instance ID.
        """
        secret = (self.webhook_secret or self.api_key or "").strip()
        if not secret:
            return False
        ...
        if auth_header and hmac.compare_digest(auth_header, secret):
            return True

        # 2. HMAC-SHA256 signature check
        sig = headers.get("x-papi-signature") or webhook_data.get("signature", "")
        if sig:
            raw_payload = body.encode("utf-8") if body else json.dumps(webhook_data, sort_keys=True).encode("utf-8")
            expected_sig = hmac.new(secret.encode("utf-8"), raw_payload, hashlib.sha256).hexdigest()
            if hmac.compare_digest(sig.strip().lower(), expected_sig.lower()):
                return True

        # 3. Instance ID validation
        if self.instance_id and (
            webhook_data.get("instance_id") == self.instance_id
            or webhook_data.get("instanceId") == self.instance_id
        ):
            return True
