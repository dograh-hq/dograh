...
async def test_minimax_delayed_http_and_redis_preserve_playback_order(
    ...
    # Populate via real HTTP using the exact sentence aggregation/payloads that
    # will be replayed, then remove just the entries meant to miss.
    ...
    } == {number: int(number in cached_sentences) for number in (1, 2, 3)}
    assert cache.capture_bytes == 0
