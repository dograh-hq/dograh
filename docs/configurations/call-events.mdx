...
Use these columns with nullable fields. The adapter does not create or migrate
tables. Existing Glik `pipeline_diagnostics` event names and detail keys are
preserved.
...
| `severity` | STRING |
| `value_ms` | FLOAT |
| `node_id`, `node_name`, `detail` | STRING |

You receive one row per diagnostic event. `ts` is when the event happened, and
`detail` contains serialized JSON. Events cover speech, accepted user turns,
...
Events are sent after a call finishes, with their original timestamps. This
differs from the earlier Glik image, which flushed events while calls were
running. Capture and export are separate from the real-time feedback logs.
