...
```bash
DEV_BACKEND_PORT="$(bunx path-to-port .)"
BACKEND_URL="http://localhost:${DEV_BACKEND_PORT}"
cd ui && NEXT_PUBLIC_BACKEND_URL="$BACKEND_URL" BACKEND_URL="$BACKEND_URL" npm run dev -- --hostname 0.0.0.0 --port "$((DEV_BACKEND_PORT + 1))"
```

5. Open the app at `http://localhost:$((DEV_BACKEND_PORT + 1))`.

Backend API and Web UI defaults are now derived from `bunx path-to-port .` if no explicit `UVICORN_BASE_PORT` / `UVICORN_PORT` (backend) or `--port` (UI) is provided.
...
| --- | --- |
| **API: Uvicorn (reload)** | The FastAPI backend, with auto-reload (port from `UVICORN_BASE_PORT` / `UVICORN_PORT` in `api/.env`, default `bunx path-to-port .`) |
