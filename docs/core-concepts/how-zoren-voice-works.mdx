...
sequenceDiagram
    ...
    participant API as Zoren Voice API
...
**Telephony**
The phone infrastructure. Zoren Voice connects to your telephony provider (Twilio, Vonage, etc.) to place and receive calls. The audio streams between the caller and Zoren Voice in real time.

**Transcriber (STT)**
Converts the caller's speech to text in real time. Zoren Voice sends the audio stream to your configured speech-to-text provider and uses the transcript to drive both the LLM and the final run record.
...
1. Zoren Voice instructs your telephony provider to dial the number
...
6. When an edge condition is met, Zoren Voice transitions to the next node
