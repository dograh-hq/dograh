...
Browsers treat microphone access as a secure-context feature, so the deployed app must be served over HTTPS (or accessed at localhost/127.0.0.1 over HTTP, which is a secure context). Local Docker uses `http://localhost:3010` and therefore does not need a certificate.
