...
<Note>
Support diagnostics for self-hosted deployments is configured through the deployment bundle. If you only pull newer `dograh-api` and `dograh-ui` images while keeping an older `docker-compose.yaml`, the stack keeps the older behavior and does not automatically receive Dograh's Sentry DSN. Use the update flow above, or refresh `docker-compose.yaml`, to make support diagnostics available. Existing `ENABLE_TELEMETRY=false` opt-outs are respected unless you explicitly set `ENABLE_SUPPORT_DIAGNOSTICS=true`.
</Note>
