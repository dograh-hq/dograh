...
description: "Complete reference for all environment variables used by the Zoren Voice backend"
---
...
Zoren Voice supports two deployment modes, set via `DEPLOYMENT_MODE`:

- **OSS**: The default mode. Designed for self-hosted deployments using [Docker Compose](/deployment/docker) — the fastest way to get Zoren Voice running. Uses local JWT authentication and MinIO for storage.
- **SaaS**: Intended for customised deployments outside of Docker. Authentication and API key management are handled through Zoren Voice Managed Platform Services (MPS), allowing greater flexibility in how the platform is hosted and integrated.
...
For Supabase or any other managed Postgres provider, keep `DATABASE_URL` in SQLAlchemy `asyncpg` format. If the runtime is IPv4-only, prefer the Supabase session pooler host; direct database connections are best on IPv6-capable hosts.

Example:

```text
postgresql+asyncpg://postgres.<project-ref>:[PASSWORD]@aws-1-ap-southeast-2.pooler.supabase.com:5432/postgres
```

When deploying with the GitHub Actions release workflow, place that same full URL in the `VOICE_DATABASE_URL` secret. If the secret is omitted, the workflow continues to use the bundled in-cluster Postgres deployment.
...
| `MPS_API_URL` | `https://services.zoren-voice.ashtra.ai` | Zoren Voice Managed Platform Services URL |
...
Zoren Voice uses **MinIO by default**, which is bundled with the self-hosted deployment and requires no external setup. Set `ENABLE_AWS_S3=true` to switch to AWS S3 — typically used for cloud or managed deployments where S3 is already part of the infrastructure.
