...
Dograh uses **MinIO as the primary artifact store**, including standard AWS EC2 deployments. Stage C can asynchronously copy finalized MinIO objects to private AWS S3 without changing call finalization, previews, or downloads.
...
| `MINIO_ALLOW_ANONYMOUS` | `true` in a direct local process; `false` in production Compose | Local compatibility switch for anonymous GET/PUT. Keep `false` in production; private MinIO uses presigned URLs. |

### AWS S3 (secondary copy, or explicit primary opt-in)

| Variable | Default | Description |
|---|---|---|
| `ENABLE_AWS_S3_PRIMARY` | `false` | Explicitly use S3 instead of MinIO as the primary backend. Leave `false` for CALMOS deployments. |
| `ENABLE_AWS_S3_SECONDARY` | `false` | Stage C: asynchronously copy successful MinIO artifacts to a private secondary S3 bucket without changing the primary backend |
| `S3_BUCKET` | `null` | S3 bucket name |
| `AWS_RECORDINGS_BUCKET` | `null` | Private S3 bucket for durable CALMOS call recordings. Falls back to `S3_BUCKET` |
| `AWS_REGION` | `eu-west-2` | AWS region. `S3_REGION` remains supported as a legacy alias |
| `AWS_S3_PREFIX` | `null` | Optional prefix for Stage C secondary objects; the default preserves the MinIO logical object key exactly |
| `S3_KMS_KEY_ID` | `null` | Optional customer-managed KMS key ID/ARN. Recordings use server-side `aws:kms` encryption |
| `S3_SERVER_SIDE_ENCRYPTION` | empty | Optional S3 server-side encryption setting; otherwise the bucket default applies |
...
In AWS production, credentials come from the API/worker instance or task IAM role. Do not place long-lived AWS access keys in the application `.env`.

### Durable call data and Sakinah memory

CALMOS Connect stores structured call data in PostgreSQL, audio/transcript objects in private MinIO, and Sakinah memory vectors in PostgreSQL `pgvector`. When Stage C is enabled, MinIO objects are copied asynchronously to private S3 after primary persistence. Database connections should use the existing TLS-enabled `DATABASE_URL`; AWS deployments should use workload IAM roles with least-privilege access to the secondary recordings bucket.

| Variable | Default | Description |
|---|---|---|
| `MEMORY_ENABLED` | `true` | Enable privacy-bounded Sakinah caller lookup and asynchronous memory extraction |
| `MEMORY_EMBEDDING_MODEL` | `text-embedding-3-small` | Embedding model used when the organization has embeddings configured |
| `MEMORY_MAX_RESULTS` | `5` | Maximum permitted memories added to a Sakinah context |
| `MEMORY_MIN_SIMILARITY` | `0.72` | Minimum cosine similarity for vector memory retrieval |
| `CALLER_IDENTIFIER_HASH_KEY` | Falls back to `OSS_JWT_SECRET` | Stable deployment secret used to HMAC caller identifiers. Set an independent value in production and do not rotate it without a re-key plan. |
| `RECORD_CALLS` | `true` | Capture and persist call recordings; transcripts remain structured call data |

#### S3-compatible servers (MinIO, rustfs, Ceph, ...)

The S3 backend can target any S3-compatible server, not just AWS. Both the native MinIO backend and the generic S3 backend issue presigned URLs when their buckets are private.

To use it as an intentional primary backend, set `ENABLE_AWS_S3_PRIMARY=true` and point it at your server with the `S3_*` overrides above. For example, against [rustfs](https://github.com/rustfs/rustfs):

```bash
ENABLE_AWS_S3_PRIMARY=true
