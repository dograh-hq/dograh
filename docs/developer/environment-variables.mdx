...
- **Local**: The default mode, designed for private deployments using [Docker Compose](/deployment/docker) — the fastest way to get Dograh running. Uses local JWT authentication and MinIO for storage.
...
| `DEPLOYMENT_MODE` | `oss` | Deployment mode. Use `oss` for local/private deployments |
...
## Authentication (Local/Private)

| Variable | Default | Description |
|---|---|---|
| `OSS_JWT_SECRET` | N/A | **Required for local/private deployments.** Secret used to sign JWT tokens. Must be set to a strong random value in production |
...
| `CORS_ALLOWED_ORIGINS` | `null` | **Required for managed deployments.** Comma-separated list of origins allowed to make credentialed cross-origin requests (e.g. `https://app.example.com,https://admin.example.com`). Ignored in local/private mode, which serves a permissive same-origin policy without credentials |

---

## Storage

Dograh uses **MinIO by default**, which is bundled with local/private deployments and requires no external setup. Set `ENABLE_AWS_S3=true` to switch to AWS S3 — typically used for managed deployments where S3 is already part of the infrastructure.

### MinIO (Local/private default)
...
| `MINIO_ACCESS_KEY` | N/A | **Required for local/private deployments.** MinIO access key. Must be set to a secure value in production |
| `MINIO_SECRET_KEY` | N/A | **Required for local/private deployments.** MinIO secret key. Must be set to a secure value in production |
