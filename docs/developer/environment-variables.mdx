...
CALMOS Connect stores structured call data in the configured PostgreSQL database/RDS instance, audio in the private S3 bucket, and Sakinah memory vectors in PostgreSQL `pgvector`. Database connections should use the existing TLS-enabled `DATABASE_URL`; AWS deployments should use IAM task roles with least-privilege access to the recordings bucket and KMS key.
