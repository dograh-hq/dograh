...
- A Dograh API key. Generate one at [`/api-keys`](https://app.dograh.com/api-keys) (or `http://localhost:3010/api-keys` for private deployments). See [API Keys](/configurations/api-keys).
- Your Dograh MCP endpoint:
  - Hosted: `https://app.dograh.com/api/v1/mcp/`
  - Private: `<YOUR_BACKEND_URL>/api/v1/mcp/`
...
Replace `YOUR_API_KEY` with the key you generated. For private deployments, swap the URL for your backend.
...
Replace `YOUR_API_KEY` with the key you generated. For private deployments, replace the URL with your backend MCP endpoint.
