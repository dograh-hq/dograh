...
{/* WALKTHROUGH VIDEO — placeholder.
    To publish: replace VIDEO_ID with the YouTube ID of the walkthrough
    (the part after `youtu.be/` or `?v=`), then delete this comment's opening
    line and the closing marker so the iframe renders. Nothing else changes.
