...
description: "Connect Zoren Voice AI to your Asterisk PBX using the Asterisk REST Interface (ARI)"
---

## Overview

Asterisk ARI (Asterisk REST Interface) allows you to connect Zoren Voice AI voice agents to your existing Asterisk PBX. ARI provides a WebSocket-based event model for controlling calls via Stasis applications, giving Zoren Voice full control over call flow and audio streaming.

This guide focuses on the Zoren Voice-specific configuration. For general Asterisk installation and administration, refer to the [official Asterisk documentation](https://docs.asterisk.org/).
...
- Network connectivity between your Zoren Voice instance and Asterisk
- Zoren Voice AI instance running and accessible

<Note>
If you compiled Asterisk from source, ensure `chan_websocket` is included during the build. This module is required for external media streaming between Asterisk and Zoren Voice. Refer to the [Asterisk build system documentation](https://docs.asterisk.org/) for details on enabling modules.
</Note>

## Asterisk Configuration

The following Asterisk configuration files need to be set up to work with Zoren Voice. These are minimal examples focused on the Zoren Voice integration -- refer to the [Asterisk documentation](https://docs.asterisk.org/) for full configuration details.

### Enable ARI (`ari.conf`)

Create an ARI user that Zoren Voice will use to authenticate:
...
<Note>
The username (section name, e.g., `dograh`) and password here must match the **Stasis App Name** and **App Password** you configure in Zoren Voice.
</Note>
...
Route incoming calls to your Stasis application so Zoren Voice can handle them:
...
Replace `dograh` with the app name you configured in `ari.conf` and in Zoren Voice.

### Configure External Media Streaming (`websocket_client.conf`)

Zoren Voice uses Asterisk's external media streaming to send and receive audio over WebSocket. Configure a WebSocket client connection that points to your Zoren Voice instance:
...
<Note>
The section name (e.g., `dograh_staging`) is the **WebSocket Client Name** you'll enter in the Zoren Voice telephony configuration. This name tells Asterisk which WebSocket connection to use for external media streaming during calls.
</Note>
...
Unlike other telephony providers that use HTTP webhooks for inbound calls, ARI delivers inbound calls as **StasisStart events on the ARI WebSocket**. Zoren Voice automatically detects these events and activates the configured workflow.
...
3. Zoren Voice identifies this as an inbound call, validates your quota, and creates a workflow run
...
**Step 2: Set the Inbound Workflow ID in Zoren Voice**
...
<AccordionGroup>
  <Accordion title="Cannot connect to ARI endpoint">
    - Verify the ARI endpoint URL is correct and reachable from your Zoren Voice instance
  ...
  <Accordion title="No audio during calls">
    ...
    - Check that `websocket_client.conf` is correctly configured with the right Zoren Voice URI
    - Ensure the WebSocket Client Name in Zoren Voice matches the section name in `websocket_client.conf`
    - Verify network connectivity and firewall rules allow WebSocket traffic between Asterisk and Zoren Voice
  </Accordion>

  <Accordion title="Calls not reaching Zoren Voice">
  ...
  <Accordion title="Inbound calls are immediately hung up">
    ...
    - Review Zoren Voice logs for warnings mentioning "no inbound_workflow_id configured"
  </Accordion>

  <Accordion title="WebSocket client connection issues">
    - Check the URI in `websocket_client.conf` points to the correct Zoren Voice host and port
    - Verify the Zoren Voice instance is running and accepting WebSocket connections
...
- Keep your Asterisk instance on the same network or a low-latency connection to Zoren Voice for optimal audio quality
- Use strong passwords for ARI authentication
- Restrict ARI access to known IP addresses using firewall rules
- Monitor Asterisk logs alongside Zoren Voice logs when debugging call issues
