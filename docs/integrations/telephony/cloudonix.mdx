...
Cloudonix routes inbound calls per **Voice Application** — the webhook URL is set once on the application, and applies to every DNID bound to it. Dograh updates that application's `url` programmatically whenever you assign an inbound workflow to a number, so you don't need to set the webhook by hand.

### Step 1: Set Up the Inbound Trunk and DNIDs
...
3. Bind each DNID (phone number) you want to use for inbound to the same Voice Application whose name you configured in Dograh
...
4. Save
   ...
   <Note>
     On save, Dograh updates the `url` on your Cloudonix Voice Application to
     `https://api.dograh.com/api/v1/telephony/inbound/run` (with `method=POST`).
     The same URL is shared across every DNID bound to that application —
     Dograh routes each inbound call to the right agent based on the called
     number's inbound workflow assignment. If you're using self-hosted Dograh,
     `api.dograh.com` is replaced with your domain.
   </Note>
