...
<iframe
  ...
  src="https://www.youtube.com/embed/aJDCw4RTl14"
...
For Indian numbers, Dograh's `_exotel_dial_number()` helper maps E.164 to Exotel's 0-prefixed national form for both `From` and `CallerId` (Exotel's Connect examples use `07314852338`, not `+917314852338`). Example: `+917314852338` → `07314852338`. Non-India numbers pass through unchanged.
...
3. Configure the applet:
   ...
   - **Sample Rate** = `8000` (default), or append `?sample-rate=16000` / `?sample-rate=24000` for enhanced/HD.
...
- **Sample rates**: `8000` (default, PSTN), `16000`, or `24000` (HD). Pass via `?sample-rate=…` on the applet URL.
