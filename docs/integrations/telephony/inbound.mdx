...
description: "Configure inbound calling for Twilio, Vonage, and Cloudonix providers in Zoren Voice AI"
---

## Overview

Zoren Voice AI supports inbound calling across all supported telephony providers. When someone calls your configured phone number, your voice agent automatically answers and handles the interaction. The inbound calling configuration in Zoren Voice AI uses the same telephony settings as outbound calling.
...
2. **Webhook Triggered**: The provider sends a webhook to Zoren Voice AI
3. **Agent Activated**: Zoren Voice AI activates your configured voice agent workflow
...
## Configuration in Zoren Voice AI
...
1. **Find Your Workflow ID**: 
   - Navigate to your workflow page (e.g., `https://zoren-voice.ashtra.ai/workflow/3`)
   - Note the workflow ID from the URL (in this example: `3`)

2. **Construct Webhook URL**:
   ```
   https://api.zoren-voice.ashtra.ai/api/v1/telephony/inbound/{workflow_id}
   ```
   
   For example, if your workflow ID is `3`:
   ```
   https://api.zoren-voice.ashtra.ai/api/v1/telephony/inbound/3
   ```

   <Note>
   If you're using a self-hosted Zoren Voice instance, replace `api.zoren-voice.ashtra.ai` with your own domain.
   </Note>

### Step 3: Configure Provider-Specific Inbound Settings

Each telephony provider requires additional configuration to route incoming calls to your Zoren Voice AI webhook. Follow the provider-specific inbound setup instructions:
...
After completing both the Zoren Voice AI configuration and provider-specific setup:
...
3. **Check Logs**: Monitor call logs in both Zoren Voice AI and your provider's dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Provider mismatch error">
    ...
    - Ensure the phone number is configured for the same provider in both Zoren Voice AI and your telephony console
  </Accordion>
  ...
  <Accordion title="Workflow not found error">
    ...
    - Double-check the webhook URL format: `https://api.zoren-voice.ashtra.ai/api/v1/telephony/inbound/{workflow_id}`
  </Accordion>
  
  <Accordion title="Account validation failed">
    - **Error**: "Authentication error: Account credentials do not match"
    - **Solution**: Verify your account SID/credentials configuration in Zoren Voice AI matches your telephony provider settings
  ...
  <Accordion title="Phone number not configured">
    ...
    - **Solution**: Add this phone number to your telephony configuration in Zoren Voice AI
  ...
  <Accordion title="Signature validation failed">
    ...
    - **Solution**: Verify your auth token configuration in Zoren Voice AI matches your provider settings
  ...
  <Accordion title="Poor audio quality on inbound calls">
    - Check network bandwidth and latency from provider to Zoren Voice AI
...
- **Monitoring**: Monitor both Zoren Voice AI logs and provider dashboards for call analytics
