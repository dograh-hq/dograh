...
Inbound routing is driven by the phone number itself — there is a **single webhook URL** for the entire org, and Dograh resolves which agent to run from the called number's assigned inbound workflow.
...
### Step 2: Configure the Webhook in Twilio Console
...
3. In the **Voice Configuration** section:
   - Set **Webhook** to: `https://api.dograh.com/api/v1/telephony/inbound/run`
   - Set **HTTP Method** to: `POST`
4. Click **Save Configuration**

   <Note>
     The webhook URL is the same for every workflow and every number in your org
     — Dograh matches the inbound call to the right agent using the called
     number's inbound workflow assignment from Step 1. If you're using
     self-hosted Dograh, replace `api.dograh.com` with your domain.
   </Note>
