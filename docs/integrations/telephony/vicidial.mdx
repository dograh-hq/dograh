...
Dograh records every call's outcome on the VICIdial lead's `status` column, so
a call Dograh handled is dispositioned in the same reports as one an agent
handled. Dograh's own disposition names (`user_hangup`, `voicemail_detected`)
are not VICIdial status codes and do not fit `vicidial_list.status`, which is
six characters, so they have to be mapped onto the codes your deployment uses.

1. Open **Platform Settings** and enable **Disposition mapping** under
   Preferences.
2. Select **Configure mapping**.
3. Set each Dograh disposition to the VICIdial status code you want recorded.
   A disposition left as itself is sent unchanged.
4. Select **Add disposition** for any end-call tool reason your agent emits
   that is not in the built-in list, then save your preferences.

For example, map `call_transferred` to `XFER`, `user_hangup` to `HUNGUP`, and
`voicemail_detected` to `AA`. VICIdial exposes no API for listing a
deployment's configured statuses, so the codes come from your own VICIdial
status report rather than being discovered automatically.

The mapped code is written to `gathered_context.mapped_call_disposition`, which
is also what outbound webhooks, the disposition run filter, and reports read.
...
An unmapped disposition is not written at all: VICIdial keeps whatever it
recorded for the call, usually `RAXFER`. That is deliberate — an unrecognised
outcome written as a plausible-looking code is worse than one that is visibly
missing when someone reads the report.
