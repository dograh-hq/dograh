...
Telnyx, Asterisk ARI, and Exotel outbound have no answer webhook. Telnyx is call-control style — Dograh POSTs the stream and event URLs to Telnyx's API instead of returning markup. ARI streams over a WebSocket only. Exotel Connect Voice AI attaches `StreamUrl` at dial time.
