...
Generate an API key at [`/api-keys`](https://app.dograh.com/api-keys) (or `http://localhost:3010/api-keys` for private deployments). See [API Keys](/configurations/api-keys) for details.
...
<Note>
For private deployments, swap `baseUrl` for your backend URL (e.g. `http://localhost:8000`).
</Note>
