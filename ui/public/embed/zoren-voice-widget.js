/**
 * Zoren Voice Widget
 * Embeddable voice call widget for Zoren Voice workflows
 * Version: 1.0.0
 */
...
(function() {
  ...
  async function init() {
    ...
    if (!token) {
      console.error('Zoren Voice Widget: No token found in script URL');
      return;
    }
    ...
    state.config = {
      ...
      contextVariables: parseContextVariables(script.getAttribute('data-zoren-voice-context'))
    };
    ...
    try {
      ...
      state.config = {
        ...
        containerId: configData.settings?.containerId || 'zoren-voice-inline-container',
        position: configData.position || DEFAULT_CONFIG.position,
        autoStart: configData.auto_start || false
      };
    } catch (error) {
      console.error('Zoren Voice Widget: Failed to fetch configuration', error);
      return;
    }
  ...
  function parseContextVariables(contextStr) {
    ...
    try {
      ...
      console.warn('Zoren Voice Widget: Invalid context variables', e);
      return {};
    }
  }
  ...
  function injectStyles() {
    if (document.getElementById('zoren-voice-widget-styles')) return;

    const styles = `
      .zoren-voice-widget-container {
        position: fixed;
        z-index: 999999;
        font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif;
      }

      .zoren-voice-widget-container.bottom-right {
        bottom: 20px;
        right: 20px;
      }

      .zoren-voice-widget-container.bottom-left {
        bottom: 20px;
        left: 20px;
      }

      .zoren-voice-widget-container.top-right {
        top: 20px;
        right: 20px;
      }

      .zoren-voice-widget-container.top-left {
        top: 20px;
        left: 20px;
      }

      .zoren-voice-widget-button {
        color: white;
        border: none;
        border-radius: 50%;
        width: 60px;
      ...
      .zoren-voice-widget-button:hover {
        transform: scale(1.1);
        box-shadow: 0 6px 20px rgba(0, 0, 0, 0.25);
      }

      .zoren-voice-widget-button:active {
        transform: scale(0.95);
      }

      /* Green button for idle/ready state */
      .zoren-voice-widget-button-idle {
        background: #10b981;
      }

      .zoren-voice-widget-button-idle:hover {
        background: #059669;
      }

      /* Orange button for connecting state */
      .zoren-voice-widget-button-connecting {
        background: #f59e0b;
        animation: pulse 2s infinite;
      }

      /* Red button for connected state (to end call) */
      .zoren-voice-widget-button-connected {
        background: #ef4444;
      }

      .zoren-voice-widget-button-connected:hover {
        background: #dc2626;
      }

      /* Red button for failed state */
      .zoren-voice-widget-button-failed {
        background: #ef4444;
        opacity: 0.8;
      }
    ...
    styleSheet.id = 'zoren-voice-widget-styles';
  ...
  function createFloatingWidget() {
    ...
    container.className = `zoren-voice-widget-container ${state.config.position}`;
    container.id = 'zoren-voice-widget';

    // Create button (green to start, red to end)
    const button = document.createElement('button');
    button.className = 'zoren-voice-widget-button zoren-voice-widget-button-idle';
    button.id = 'zoren-voice-widget-button';
    button.innerHTML = `
    ...
    audio.id = 'zoren-voice-widget-audio';
    audio.autoplay = true;
  ...
  function updateFloatingButton(status) {
    const button = document.getElementById('zoren-voice-widget-button');
    if (!button) return;

    // Remove all status classes
    button.classList.remove('zoren-voice-widget-button-idle', 'zoren-voice-widget-button-connecting', 'zoren-voice-widget-button-connected', 'zoren-voice-widget-button-failed');

    // Add current status class
    button.classList.add(`zoren-voice-widget-button-${status}`);
  ...
  function createInlineWidget() {
    ...
    if (!container) {
      console.error(`Zoren Voice Widget: Container element with id "${state.config.containerId}" not found`);
  ...
  function getStatusIcon(status) {
    const icons = {
      ...
      connecting: `<svg class="zoren-voice-widget-spinner" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <path d="M12 2v4"/>
        <path d="M12 18v4"/>
  ...
  async function startCall() {
    ...
    try {
      ...
      console.error('Zoren Voice Widget: Failed to start call', error);
  ...
  async function fetchTurnCredentials() {
    if (!state.sessionToken) {
      console.warn('Zoren Voice Widget: No session token available for TURN credentials');
      return;
    }
  ...
  const widgetApi = {
    ...
    initInline: (options) => {
      if (options.container) {
        state.config.containerId = options.container.id || 'zoren-voice-inline-container';
      }
  ...
  window["\x44\x6f\x67\x72\x61\x68Widget"] = widgetApi;
  window.ZorenVoiceWidget = widgetApi;
