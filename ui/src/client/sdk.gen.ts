...
/**
 * Get Order Status
 * Get order status and refund information.
 *
 * This is a dummy endpoint for testing voice agent function calls.
 * It simulates processing time with a 10-second delay.
 *
 * Args:
 * request: Request containing the order ID to look up
 *
 * Returns:
 * Order details including refund status
 */
export const getOrderStatusApiV1OrderStatusPost = <ThrowOnError extends boolean = false>(options: Options<GetOrderStatusApiV1OrderStatusPostData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<GetOrderStatusApiV1OrderStatusPostResponse, GetOrderStatusApiV1OrderStatusPostError, ThrowOnError>({
