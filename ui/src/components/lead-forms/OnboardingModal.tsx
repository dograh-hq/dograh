    ...
    void (async () => {
      try {
        ...
        if (efSnapshot) {
          ...
          toast.success(t("successCheckInbox"));
        }
      } catch {
  ...
  const handleSubmit = () => {
    ...
    if (wantsOnPrem) {
      ...
      if (!ef.name.trim() || !ef.company.trim() || !ef.jobTitle.trim() || !ef.phone.trim() || !ef.volume) {
        toast.error(t("errorCompleteOnPrem"));
        return;
      }
    ...
    >
      <div className="grid gap-4">
        <div className="space-y-1.5">
          <Label htmlFor="ob-persona">{t("whatBestDescribesYou")}</Label>
          <Select
            value={persona}
            onValueChange={(v) => {
              setPersona(v);
          ...
          >
            <SelectTrigger id="ob-persona"><SelectValue placeholder={t("selectOne")} /></SelectTrigger>
            <SelectContent>
        ...
        {showOnPrem && (
          <div className="space-y-1.5">
            <Label htmlFor="ob-onprem">{t("needOnPrem")}</Label>
            <Select
              value={onPremNeed}
              onValueChange={(v) => {
                setOnPremNeed(v);
            ...
            >
              <SelectTrigger id="ob-onprem"><SelectValue placeholder={t("selectOne")} /></SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="ob-volume">{t("monthlyCallVolume")}</Label>
          <Select value={volume} onValueChange={setVolume}>
            <SelectTrigger id="ob-volume"><SelectValue placeholder={t("selectOne")} /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="ob-migrating">{t("migratingFrom")}</Label>
          <Select
            value={migratingFrom}
            onValueChange={(v) => {
              setMigratingFrom(v);
          ...
          >
            <SelectTrigger id="ob-migrating"><SelectValue placeholder={t("selectOne")} /></SelectTrigger>
            <SelectContent>
          ...
          {isOtherProvider && (
            <div className="mt-2 space-y-1.5">
              <Label htmlFor="ob-other-provider">{t("enterOtherProvider")}</Label>
              <Input
                id="ob-other-provider"
                placeholder={t("enterOtherProvider")}
          ...
          {isMigrating && (
            <div className="mt-2 space-y-1.5">
              <Label htmlFor="ob-switch-reason">
                {t("whySwitching")} <span className="text-muted-foreground">{t("optional")}</span>
              </Label>
              <Textarea
                id="ob-switch-reason"
                rows={2}
                placeholder={t("switchReasonPlaceholder")}
                value={switchReason}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="ob-heard">{t("howHeard")}</Label>
          <Select value={howHeard} onValueChange={setHowHeard}>
            <SelectTrigger id="ob-heard"><SelectValue placeholder={t("selectOne")} /></SelectTrigger>
            <SelectContent>
