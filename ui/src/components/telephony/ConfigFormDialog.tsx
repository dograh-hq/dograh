...
export function ConfigFormDialog({
  ...
  const t = useTranslations("telephonyForm");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="cfg-provider">{t("provider")}</Label>
            <Select
              value={providerName}
            ...
            {currentProvider?.docs_url && (
              ...
              >
                {t("providerDocs", { name: currentProvider.display_name })} <ExternalLink className="h-3 w-3" />
              </a>
            )}
          </div>
          ...
          {!isEdit && (
            <div className="flex items-center justify-between rounded border p-3">
              <div>
                <Label className="text-sm">{t("setDefault")}</Label>
                <p className="text-xs text-muted-foreground">
                  {t("setDefaultDesc")}
                </p>
              </div>
...
function FieldInput({ field, value, onChange, isEdit, t }: FieldInputProps) {
  if (field.name === "from_numbers") {
    return (
      <p className="text-xs text-muted-foreground">
        {t("phoneNumbersSeparate")}
      </p>
    );
  }
  ...
  const placeholder =
    field.placeholder ??
    (field.sensitive && isEdit ? t("leaveMasked") : "");
