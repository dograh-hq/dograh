...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isZorenVoiceDomain =
    window.location.hostname === 'zoren-voice.ashtra.ai' ||
    window.location.hostname.endsWith('.zoren-voice.ashtra.ai');
  const cookieDomainPart = isZorenVoiceDomain ? '; domain=.zoren-voice.ashtra.ai' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.zoren-voice.ashtra.ai). The route will set the
  ...
  // superadmin.zoren-voice.ashtra.ai we want to switch to app.zoren-voice.ashtra.ai. For any other domain
